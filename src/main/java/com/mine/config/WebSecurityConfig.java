...
//		.headers(headers -> headers.httpStrictTransportSecurity(hsts -> hsts.disable()))
        ...
        .exceptionHandling((exception)-> exception.authenticationEntryPoint(new RestAuthenticationEntryPoint()).accessDeniedHandler(new CustomAccessDeinedHandler()))    
        .authorizeHttpRequests(auth ->
        	auth
        	.requestMatchers("/", "/error","/api/all", "/api/auth/**", "/oauth2/**")
        	.permitAll()
        	.requestMatchers("/api/v1/**").hasAuthority(UserRoleEnum.ROLE_USER.name())
        	.anyRequest()
        	.authenticated())
        
        .oauth2Login(oauth2 -> 
        	oauth2.authorizationEndpoint(authEndpoint -> authEndpoint.authorizationRequestRepository(cookieAuthorizationRequestRepository()))
        	.redirectionEndpoint(Customizer.withDefaults())
        	.userInfoEndpoint(userInfoEndpoint -> userInfoEndpoint.oidcUserService(customOidcUserService).userService(customOAuth2UserService))
